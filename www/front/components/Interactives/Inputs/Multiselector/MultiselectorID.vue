        ...
        .multiselector__wrapper
          label.multiselector__input-container(
            ...
            input.multiselector__input(
              ...
              @keydown.tab="changeIsOpen(false)"
            )
          ...
          label.multiselector__value-container(
            ...
            .multiselector__value(v-if="!multiply && !valueIsEmpty")
              slot(name="value" :opt="getSelected") {{ customName(getSelected) }}
...
export default {
    ...
    getSelected() {
      if (this.multiply) {
        ...
        this.value.forEach(k => {
          selected.push(this.options.find(opt => opt[this.byId] === k))
        })
        return selected
      }
  ...
  methods: {
    ...
    isSelected(opt) {
      if (this.multiply) {
        const index = this.value.findIndex(k => k === opt[this.byId])
        return index !== -1
      }
    ...
    select(opt) {
      if (this.multiply) {
        this.value.push(opt[this.byId])
      } else {
        this.$emit('update:value', opt[this.byId])
      }
